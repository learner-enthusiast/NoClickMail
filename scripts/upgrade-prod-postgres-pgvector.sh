...
#
# Requires in .env:
#   DATABASE_URL (or DATABASE_URL_DIRECT) — db name, password, host, port read from this (for migrate)
#   POSTGRES_USER — optional; Docker superuser for exec/backup (default: postgres, NOT parsed from URL)
#   POSTGRES_DB / POSTGRES_PASSWORD — optional overrides
...
# Explicit .env overrides only — do not infer Docker superuser from DATABASE_URL app user.
SAVED_POSTGRES_USER="${POSTGRES_USER:-}"
SAVED_POSTGRES_DB="${POSTGRES_DB:-}"
SAVED_POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-}"
SAVED_POSTGRES_PORT="${POSTGRES_PORT:-}"
...
# Docker volume superuser (existing prod volumes are almost always "postgres").
DOCKER_POSTGRES_USER="${SAVED_POSTGRES_USER:-postgres}"
POSTGRES_USER="${DOCKER_POSTGRES_USER}"
POSTGRES_DB="${SAVED_POSTGRES_DB:-${PARSED_DB_NAME}}"
POSTGRES_PORT="${SAVED_POSTGRES_PORT:-${PARSED_DB_PORT}}"
POSTGRES_PASSWORD="${SAVED_POSTGRES_PASSWORD:-${PARSED_DB_PASSWORD:-}}"
...
log "DATABASE_URL: ${PARSED_DB_HOST}:${POSTGRES_PORT}/${POSTGRES_DB} (app user: ${PARSED_DB_APP_USER})"
log "Docker postgres superuser: ${DOCKER_POSTGRES_USER}"

docker_psql() {
  docker compose -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" exec -T \
    -e PGPASSWORD="${POSTGRES_PASSWORD}" postgres \
    psql -h 127.0.0.1 -U "${DOCKER_POSTGRES_USER}" "$@"
}

docker_pg_dump() {
  docker compose -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" exec -T \
    -e PGPASSWORD="${POSTGRES_PASSWORD}" postgres \
    pg_dump -h 127.0.0.1 -U "${DOCKER_POSTGRES_USER}" "$@"
}
...
for _ in $(seq 1 30); do
  if docker compose -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" exec -T postgres \
    pg_isready -h 127.0.0.1 -U "${DOCKER_POSTGRES_USER}" -d "${POSTGRES_DB}" >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
...
docker compose -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" exec -T postgres \
  pg_isready -h 127.0.0.1 -U "${DOCKER_POSTGRES_USER}" -d "${POSTGRES_DB}"
