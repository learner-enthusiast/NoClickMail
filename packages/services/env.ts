...
const envSchema = z.object({
  ...
  GOOGLE_OAUTH_REDIRECT_URI: z.preprocess(emptyToUndefined, z.string().optional()),
  ...
  CORSAIR_GMAIL_REDIRECT_URI: z.preprocess(emptyToUndefined, z.string().url().optional()),
  /** User-facing redirect after Calendar connect — defaults to ${CLIENT_URL}/dashboard/calendar */
  CORSAIR_CALENDAR_REDIRECT_URI: z.preprocess(emptyToUndefined, z.string().url().optional()),
  ...
  INNGEST_SIGNING_KEY: z.preprocess(emptyToUndefined, z.string().min(1).optional()),
  /** Shared cookie domain for web + API subdomains (e.g. .arnabsamanta.in). Auto-derived from CLIENT_URL when unset. */
  COOKIE_DOMAIN: z.preprocess(emptyToUndefined, z.string().min(2).optional()),
});
